...
@register
@serializable
class YOLOv3MixUnetFPN(nn.Layer):
    __shared__ = ['norm_type', 'data_format']
    ...
    def __init__(self,
        ...
        """
        super(YOLOv3MixUnetFPN, self).__init__()
        assert len(in_channels) > 0, "in_channels length should > 0"
        self.in_channels = in_channels
        self.num_blocks = len(in_channels)
    ...
    def forward(self, blocks, for_mot=False):
        ...
        if for_mot:
            emb_feats = []
        for i, block in enumerate(blocks):
            # blocks 为骨架网络输入过来的tensor元组 每一个block为各层输出
            print(block.shape)
        for i, block in enumerate(blocks):
            # blocks 为骨架网络输入过来的tensor元组 每一个block为各层输出
            print(block.shape)
...
@register
@serializable
class YOLOMixUnetCSPPAN(nn.Layer):
    """
    YOLO CSP-PAN, used in YOLOv5 and YOLOX.
    """
    __shared__ = ['depth_mult', 'data_format', 'act', 'trt']
    ...
    def __init__(self,
                 ...
                 act='silu',
                 trt=False):
        super(YOLOMixUnetCSPPAN, self).__init__()
        self.in_channels = in_channels
        self._out_channels = in_channels
        Conv = DWConv if depthwise else BaseConv
