...
__all__ = ['YOLOv3Head', 'YOLOXHead', 'YOLOXMixUnetHead']
...
@register
class YOLOv3MixUnetHead(nn.Layer):
    __shared__ = ['num_classes', 'data_format']
    __inject__ = ['loss']

    def __init__(self,
                 in_channels=[192, 96, 48, 24],
                 anchors=[[10, 13], [16, 30], [33, 23], [30, 61], [62, 45],
                          [59, 119], [116, 90], [156, 198], [200, 300], [300, 350], [350, 370], [373, 326]],
        ...
        Args:
            ...
            data_format (str): data format, NCHW or NHWC
        """
        super(YOLOv3MixUnetHead, self).__init__()
        assert len(in_channels) > 0, "in_channels length should > 0"
        self.in_channels = in_channels
        self.num_classes = num_classes
        self.loss = loss
...
@register
class YOLOXMixUnetHead(nn.Layer):
    __shared__ = ['num_classes', 'width_mult', 'act', 'trt', 'exclude_nms']
    __inject__ = ['assigner', 'nms']
    ...
    def __init__(self,
                 ...
                 trt=False,
                 exclude_nms=False):
        super(YOLOXMixUnetHead, self).__init__()
        self._dtype = paddle.framework.get_default_dtype()
        self.num_classes = num_classes
        assert len(in_channels) > 0, "in_channels length should > 0"
        self.in_channels = in_channels
